      ...
      <section className="relative overflow-hidden pt-32 pb-20 lg:pt-40 lg:pb-28">
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-12 px-5 md:px-8 lg:grid-cols-12 lg:gap-16">
          <div className="lg:col-span-7">
            ...
            <p className="mt-8 text-foreground/70 text-lg max-w-xl leading-relaxed">
          ...
          <div className="lg:col-span-5">
            <div className="grid grid-cols-2 gap-4">
              <img src="/gallery/destaque-001.jpg" alt="" className="aspect-[3/4] rounded-2xl object-cover shadow-editorial" />
              <img src="/gallery/destaque-003.jpeg" alt="" className="mt-10 aspect-[3/4] rounded-2xl object-cover shadow-editorial" />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-ivory py-20">
        <div className="mx-auto max-w-7xl px-5 md:px-8">
          <div className="grid gap-10 md:grid-cols-3">
            {[
              { icon: Heart, title: "Acolhimento", text: "Cada cliente é única, e seu tempo no estúdio é desenhado para isso." },
              { icon: Award, title: "Formação internacional", text: "Aperfeiçoamento contínuo com Pivot Point, L'Oréal Professionnel e SOFT Hair." },
              { icon: Sparkles, title: "Resultado editorial", text: "Loiros e cores autorais com técnica de alta performance." },
            ].map((v) => (
              <div key={v.title} className="rounded-2xl border bg-card p-8 shadow-luxe">
                <v.icon className="h-7 w-7 text-gold-deep" />
              ...
              >
                <div className="md:col-span-5">
                  <div className="relative">
                    ...
                    <div className="absolute -bottom-5 -right-5 hidden h-28 w-28 rounded-2xl bg-gradient-gold md:block" />
                  </div>
                </div>
                <div className="md:col-span-7">
                  <div className="eyebrow">{p.role_title}</div>
                  <h3 className="mt-3 font-serif text-5xl lg:text-6xl leading-[1.02]">{p.name}</h3>
                  <p className="mt-6 text-foreground/70 leading-relaxed text-lg max-w-xl">{p.bio}</p>
